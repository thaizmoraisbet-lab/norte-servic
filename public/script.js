...
function posicaoTocantinsPorNome(cidade) {
  if (NORTE_SERVIC_POSICOES_TOCANTINS[cidade]) return NORTE_SERVIC_POSICOES_TOCANTINS[cidade];

  // fallback visual: mantém qualquer cidade do Tocantins dentro da área do mapa,
  // mesmo que ainda não tenha coordenada manual cadastrada.
  let hash = 0;
  const texto = cidade.normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase();
  ...
  const faixa = faixas[positivo % faixas.length];
  const x = faixa.x[0] + (positivo % 1000) / 1000 * (faixa.x[1] - faixa.x[0]);
  const y = faixa.y[0] + ((positivo / 1000) % 1000) / 1000 * (faixa.y[1] - faixa.y[0]);
  return [Number(x.toFixed(1)), Number(y.toFixed(1))];
}

async function carregarMapaTocantinsPresenca() {
  const container = document.getElementById('tocantinsPresencaPontos');
  const box = document.getElementById('tocantinsPresencaMapa');
  if (!container || !box) return;

  container.innerHTML = '';

  try {
    const resposta = await fetch('/api/home/tocantins-cidades-ativas');
    if (!resposta.ok) throw new Error('Não foi possível consultar cidades ativas.');
    const dados = await resposta.json();
    const cidades = Array.isArray(dados.cidades) ? dados.cidades : [];
    const unicas = [...new Set(cidades.map(normalizarCidadeTocantins).filter(Boolean))]
      .filter((cidade) => NORTE_SERVIC_CIDADES_TOCANTINS.has(cidade));

    if (!unicas.length) {
      const vazio = document.createElement('div');
      vazio.className = 'tocantins-presenca-vazio';
      vazio.textContent = 'Cidades acendem após aprovação dos profissionais';
      box.appendChild(vazio);
      return;
    }

    const limite = unicas.slice(0, 35);
    limite.forEach((cidade, index) => {
      const [x, y] = posicaoTocantinsPorNome(cidade);
      const ponto = document.createElement('button');
      ponto.type = 'button';
      ponto.className = 'tocantins-presenca-ponto';
      ponto.dataset.cidade = cidade;
      ponto.title = `Norte Servic presente em ${cidade}`;
      ponto.style.left = `${x}%`;
      ponto.style.top = `${y}%`;
      ponto.style.animationDelay = `${(index % 8) * 0.16}s`;
      ponto.addEventListener('click', () => {
        const campo = document.getElementById('campoBusca');
        if (campo) campo.value = cidade;
        if (typeof buscarProfissionais === 'function') buscarProfissionais(cidade);
      });
      container.appendChild(ponto);
    });
  } catch (erro) {
    console.warn('Mapa Tocantins:', erro.message);
  }
}

document.addEventListener('DOMContentLoaded', carregarMapaTocantinsPresenca);
