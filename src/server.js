...
const { PUBLIC_DIR: publicDir } = require('./config/paths');
const fs = require('fs');
...
app.get('/api/profissionais', async (req, res) => {
  try {
    ...
    if (cidade) {
      ...
      where += ` AND (LOWER(cidade) LIKE $${params.length} OR LOWER(bairro) LIKE $${params.length} OR LOWER(cidades_atendidas::text) LIKE $${params.length})`;
    }
...
app.post('/api/cadastro', async (req, res) => {
  try {
    ...
    const profissionalCriado = result.rows[0];
    const token = gerarToken(profissionalCriado);
    res.status(201).json({
      mensagem: 'Conta criada com sucesso. Complete seu perfil para aumentar sua visibilidade.',
      token,
      profissional: profissionalParaFrontend(profissionalCriado)
    });
  } catch (error) {
...
app.patch('/api/me/completar', autenticar, async (req, res) => {
  try {
    await garantirSistemaWhatsAppPerfil();

    const atual = await pool.query('SELECT * FROM profissionais WHERE id=$1 LIMIT 1', [req.user.id]);
    if (!atual.rows.length) {
      return res.status(404).json({ erro: 'Profissional não encontrado.' });
    }
    ...
    const instagramBruto = String(dados.instagram || anterior.instagram || '').trim();
    const instagram = instagramBruto
    ...
    const servicos = String(dados.servicos || anterior.servicos || anterior.profissao || '').trim();
    const descricao = String(dados.descricao || anterior.descricao || '').trim();
    const perfilCompleto = Boolean(servicos && descricao.length >= 20);
    ...
    const result = await pool.query(
      ...
      [
        ...
        instagram === '@' ? '' : instagram,
        servicos,
        String(dados.bairro || anterior.bairro || '').trim(),
        String(dados.atendeOutrasCidades || dados.atende_outras_cidades || anterior.atende_outras_cidades || 'Não'),
        JSON.stringify(cidadesAtendidas.length ? cidadesAtendidas : parseJsonArray(anterior.cidades_atendidas)),
        String(dados.horarioAtendimento || dados.horario_atendimento || anterior.horario_atendimento || '').trim(),
        imagens.fotoPerfil || anterior.foto_perfil || '',
    ...
    res.json({
      mensagem: perfilCompleto
        ? 'Perfil salvo e pronto para análise.'
        : 'Perfil salvo. Você pode completar os outros dados depois.',
      profissional: profissionalParaFrontend(result.rows[0])
    });
  } catch (error) {
    res.status(500).json({ erro: 'Erro ao completar perfil.', detalhe: error.message });
  }
});
